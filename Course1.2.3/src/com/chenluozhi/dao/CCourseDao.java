package com.chenluozhi.dao;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

import com.chenluozhi.model.CCourseModel;
...
public class CCourseDao {
	...
	public boolean addCCourse(int stu_id, int course_id, String ccourse_time) {//增加选课
	 ...
	 * @param course_id
	 */
	public boolean modifyCCourse(int stu_id, int old_course_id,int new_course_id) //修改已选课程为其它
	{
	 ...
	 * @param course_id
	 */
	public boolean deleteCCourse(int stu_id,int course_id){//删除选课
	 ...
	 * @param ccourse_mark
	 */
	public boolean modifyCCourseAndMark(int stu_id,int course_id,int ccourse_mark)//修改选课成绩
	{
	 ...
	 * @return true 代表重复
	 */
	public boolean isCCourseExist(int stu_id, int course_id) //判断是否选择过该课程
	{
		String sql = "select * from c_ccourse where stu_id =" + stu_id	+ " and course_id =" + course_id;
		ResultSet rs = DBConnection.query(sql);
		try {
			if (rs.next())
				return true;//在选课中有该选课记录
	 ...
	 * @return 多行记录
	 */
	public List<CCourseModel> getListsByStuId(int stu_id,boolean isNull)//根据学号获取选课管理模块数据
	{
		
		List<CCourseModel> lists = new ArrayList<CCourseModel>();//类集框架，集合
		String sql ="";
	...
	public String getNameByCourseId(int course_id)//根据id查找对应的课程名称
	{
		String course_name = "";
	...
	public List<CCourseModel> getListsByStuId(int stu_id)//根据学号获取选课管理模块数据
	{
	 ...
	 * @return
	 */
	public List<String> getThreeMark(int course_id) //某门课程中，最高最低平均分
	{
	...
	public List<String> getGoodRadio(int course_id) {
		...
		String sql = "select count(*) as counts from c_ccourse where course_id="+ course_id; // 总人数
		ResultSet rs = DBConnection.query(sql);
		try {
			if (rs.next()) {
	...
	/**
	 * @根据查询 选课 具体情况(课程id，课程名称，学生id，学生名称，老师id，授课名称，课程成绩)
	 * @param course_id 值为-1 按课程id查询
	 * @param course_counts_index 值为-1 根据 course_id查询
	 * @return 多行记录，查询模块列表
	 */
	public List<CCourseModel> getCCourseSpecificList(int course_id,int course_counts_index)//查询被选课程的相关信息，还有没选课的学生的相关信息
	 {
		CourseDao cd = new CourseDao();
		...
		if(course_counts_index==-1)//？
			...
			case 0:		
				sql = "select stu_id  from c_student where stu_id not in  (select stu_id from c_ccourse)";	//没选课的学生id
				try {
						ResultSet rs = DBConnection.query(sql);
						while(rs.next()){
							CCourseModel ccm = new CCourseModel();
			...
			case 1:
				...
				sql = "select temp.stu_id,course_id from ((select stu_id,count(*) as counts  from c_ccourse group by stu_id having counts=1) as temp),c_ccourse where c_ccourse.stu_id = temp.stu_id";//只选了一节课的学生
				...
				break;
			case 2:
				sql = "select temp.stu_id,course_id from ((select stu_id,count(*) as counts  from c_ccourse group by stu_id having counts>=2) as temp),c_ccourse where c_ccourse.stu_id = temp.stu_id";//选了两门及以上节课的学生
