...
public class DBInit extends JDialog {//数据库初始化,清空数据
	...
	public class btListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == jButton_Start) {
				File fnew = new File("src/com/chenluozhi/db/BK_CourseChoose.mdb");//重命名文件
				File fold = new File("src/com/chenluozhi/db/CourseChoose.mdb");
				if (!fnew.exists()) {
					...
					try {                //把原备份文件复制多一份，同时改名为CourseChoose
						FileInputStream inStream = new FileInputStream("src/com/chenluozhi/db/BK_CourseChoose.mdb"); //读入原文件
						FileOutputStream fs = new FileOutputStream(new File("src/com/chenluozhi/db/CourseChoose.mdb"));
					...
					String sql_c_user = "truncate table c_user";//快速清空数据表内所有数据
							...
							for (int counter = 0; counter <= 100; counter++) {
								...
								if (counter == 100) {
									...
									JOptionPane.showMessageDialog(null,
											"谁是谁生命中的过客,谁是谁生命的转轮,\n前世的尘,今世的风,无穷无尽的哀伤的精魂.  ", "数据初始化成功", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(this.getClass().getResource("/com/chenluozhi/images/male.gif")));
								}
							}
						}
					}).start();
