...
import com.chenluozhi.util.DBInit;
...
public class MainFrame extends JFrame {
	...
	private SystemTray sysTray = SystemTray.getSystemTray();//初始化系统托盘？
	
	Image image = Toolkit.getDefaultToolkit().getImage(getClass().getResource("/com/chenluozhi/images/icon.png"));
	...
	private void initialize() {
		this.setSize(300, 160);// 主界面大小
		this.setTitle("选修课程管理系统");
		...
		this.setLocationRelativeTo(null);//设置为某组件的相对位置（会出现在屏幕中央）
		...
		// Create menu bar and set vertical layout
		jJMenuBar = new JMenuBar();
		jJMenuBar.setLayout(new BoxLayout(jJMenuBar, BoxLayout.Y_AXIS)); // Set layout to vertical
		...
		//imgURL = this.getClass().getResource("/com/chenluozhi/images/main.jpg");
			...
			if (e.getSource() == jMenuItem_relogin) {
				...
				try {
					...
					if (f.exists()) {
						...
						JOptionPane.showMessageDialog(null, "没什么好看的");
					}
				} catch (Exception e1) {
					e1.printStackTrace();
				}
