...
//	private int counts = 0;//记录数
	private JCheckBox jCheckBox_teach_filter = null;//创建一个默认的复选框
...
//	}
	...
	jTable = new JTable();//把数据以二维表格的形式显示出来，并且允许用户对表格中的数据进行编辑。

	jScrollPane = new JScrollPane();//滚动面板
	jScrollPane.setViewportView(jTable);//设置在滚动面板显示的组件
	jScrollPane.setBounds(new Rectangle(20, 90, 620, 300));

	String heads[] = {"课程ID","课程名称","学生ID","学生名称","教师ID","教师名称","课程成绩"};
	model = new DefaultTableModel(null, heads);	//声明表格模型DefaultTableModel
	...
	jPanel.setBorder(BorderFactory.createTitledBorder("操作面板"));//为panel面板设置标题
...
//				int course_index=jComboBox_course_condition.getSelectedIndex();
//				int course_id=course_lists.get(course_index).getCourse_id();
//				 course_lists=ccd.getCCourseSpecificList(course_id,-1);
//				flashTableData();
			}
